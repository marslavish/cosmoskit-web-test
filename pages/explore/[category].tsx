    ...
    >
      <Box p={4}>
        <Skeleton
          ...
          maxW={56}
          h={6}
          startColor={handleChangeColorModeValue(
            colorMode,
            'gray.50',
            'purple.700'
          )}
          endColor={handleChangeColorModeValue(
            colorMode,
            'gray.200',
            'purple.800'
          )}
...
export default function Components({ category }: { category: string }) {
  ...
  useEffect(() => {
    ...
    if (!selectedCategory) {
      const currentCategory = router.asPath.split('#')[0];
      const currentComponent = router.asPath.split('#')[1];
      if (currentComponent)
        setSelectedCategory({
      ...
      console.log('category', router.asPath);
    }
          ...
          >
            <Stack spacing={6}>
              {isLoading ? (
                ...
                displayList.map(
                  ({ displayName, component, codeString, id }) => (
                    <SnippetBlock
                      key={id}
                      id={id}
...
export function getStaticProps({ params: { category } }) {
  return { props: { category } };
}

export function getStaticPaths() {
  return {
    paths: links.map(({ category }) => ({
